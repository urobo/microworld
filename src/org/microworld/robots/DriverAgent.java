...
public class DriverAgent extends Agent implements Driver{
	...
	public Trip postTrip(Trip trip) {
		String response = DycapoHttpClient.callDycapo(DycapoHttpClient.POST, DycapoGlobalVariables.URL_BASIS + "trips/", trip.toJSONObject(), user.getUsername(), user.getPassword());
		try {
			Trip result = DycapoObjectsFetcher.buildTrip(new JSONObject(response));
			return result;
	...
	public boolean activateTrip(Trip trip) {
		trip.setActive(true);
		String response = DycapoHttpClient.callDycapo(DycapoHttpClient.PUT, trip.getHref(),trip.toJSONObject() ,user.getUsername(), user.getPassword());
		try {
			Trip active = DycapoObjectsFetcher.buildTrip(new JSONObject(response));
	...
	public void acceptRideRequests(List<Participation> list) {
		for (int i = 0 ;i < list.size(); i++){
			...
			DycapoHttpClient.callDycapo(DycapoHttpClient.PUT,list.get(i).getHref() ,list.get(i).toJSONObject(), user.getUsername(), user.getPassword());
		}
	}
	...
	public void refuseRideRequests(List<Participation> list) {
		while (!list.isEmpty()){
			DycapoHttpClient.callDycapo(DycapoHttpClient.DELETE, list.get(0).getHref(), null, user.getUsername(), user.getPassword());
			list.remove(0);
		}
	}
